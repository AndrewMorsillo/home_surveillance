...
#	self.MQTTClient.subscribe("camera/keuken/")
   ...
   def update_channel(self, channel, channelname, url):
      print("Received message2 " + str(url))
      self.cameras[channel].captureThread.stop = True
      self.cameraProcessingThreads[channel].stop = True

      #self.cameraProcessingThreads[0].join()
      self.cameras[channel] = Camera.IPCamera(channelname,url,"detect_recognise_track",False,False)
      self.cameraProcessingThreads[channel] = threading.Thread(name='frame_process_thread_' + str(channel),target=self.process_frame,args=(self.cameras[channel],))
      self.cameras[channel].captureThread.stop = False
      print("test2")
      self.cameraProcessingThreads[channel].start()
