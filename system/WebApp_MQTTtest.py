...
app.config['MQTT_BROKER_URL'] = '192.168.178.100'
...
#   print("Received message2 " + str(message.payload))
#   print(message.topic)
#   channelurl = str(message.payload)
#   cannelurl = "testing/iphoneVideos/singleTest.m4v"
#   channelnametemp = message.topic.split("/")
#   channelname = channelnametemp[1]
#   print(channelname)
#   channel = 0
#   with HomeSurveillance.camerasLock :
#      HomeSurveillance.update_channel(channel,channelname,str(message.payload))
#   return render_template('index.html')

@mqtt.on_connect()
def handle_connect(client, userdata, flags, rc):
    mqtt.subscribe('camera/keuken/')
    print("testMQTTconnection")
...
def handle_mqtt_message(client, userdata, message):
    ...
    channelurl = str(message.payload)
    channelurl = "testing/iphoneVideos/singleTest.m4v"
    channelnametemp = message.topic.split("/")
    channelname = channelnametemp[1]
    print(channelname)
    channel = 0
    with HomeSurveillance.camerasLock :
       HomeSurveillance.update_channel(channel,channelname,channelurl)
    socketio.emit('refreshscreen', "refreshscreen action" ,namespace='/surveillance')
...
@app.route('/refreshscreen', methods=['GET', 'POST'])
def refreshscreen():
    print("testrefreshscreen")
    return render_template('index.html')
